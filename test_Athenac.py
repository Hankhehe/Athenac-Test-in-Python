...
class TestNACbyIPAM:
    def test_IPBlock(self)->None:
        ...
        time.sleep(10)
        assert lan2_.ARPBlockCheck(lan2_.Ip,lan2_.gatewayIp,ProbeMAC_)\
        ,(AthenacWebAPI_.BlockIPv4(ip=lan2_.Ip,block=False,siteid=SiteID_),f'False : Not Receive ARP {lan2_.Ip}')
        lan2_.SendARPReply(IP=TestIPv4_,Count=2,WaitSec=2)
        assert not lan2_.ARPBlockCheck(TestIPv4_,lan2_.gatewayIp,ProbeMAC_)\
        ,(AthenacWebAPI_.BlockIPv4(ip=lan2_.Ip,block=False,siteid=SiteID_),f'False : Recive ARP Rqply {TestIPv4_} by Change IP')
    ...
    def test_MACblock(self)->None:
        ...
        time.sleep(10)
        assert lan2_.ARPBlockCheck(lan2_.Ip,lan2_.gatewayIp,ProbeMAC_)\
            ,(AthenacWebAPI_.BlockMAC(mac=lan2MACUpper_,block=False,siteid=SiteID_),f'False : Not Receive ARP {lan2_.Ip}')
        lan2_.SendARPReply(IP=TestIPv4_,Count=2,WaitSec=2)
        assert lan2_.ARPBlockCheck(TestIPv4_,lan2_.gatewayIp,ProbeMAC_)\
            ,(AthenacWebAPI_.BlockMAC(mac=lan2MACUpper_,block=False,siteid=SiteID_),f'False : Not Recive ARP Reply {TestIPv4_} by Change IP')
        assert lan2_.NDPBlockCheck(lan2_.globalIp,lan2_.gatewatIpv6,ProbeMAC_)\
            ,(AthenacWebAPI_.BlockMAC(mac=lan2MACUpper_,block=False,siteid=SiteID_),f'False : Not Receive NDP Adver {lan2_.globalIp}')
        lan2_.SendNA(IP=TestIPv6_,Count=2,WaitSec=2)
        assert lan2_.NDPBlockCheck(TestIPv6_,lan2_.gatewatIpv6,ProbeMAC_)\
            ,(AthenacWebAPI_.BlockMAC(mac=lan2MACUpper_,block=False,siteid=SiteID_),f'False : Not Receive NDP Adver {TestIPv6_}')
    ...
    def test_ProtectIP(self)->None:
        ...
        assert not lan1_.ARPBlockCheck(srcIP='0.0.0.0',dstIP=TestIPv4_,ProbeMAC=ProbeMAC_)\
            ,(AthenacWebAPI_.DelIP(ip=TestIPv4_,siteid=SiteID_),f'False : Recive ARP {TestIPv4_} by lan1 MAC use 0.0.0.0 check IP used {TestIPv4_}')
        assert lan2_.ARPBlockCheck(srcIP='0.0.0.0',dstIP=TestIPv4_,ProbeMAC=ProbeMAC_)\
            ,(AthenacWebAPI_.DelIP(ip=TestIPv4_,siteid=SiteID_),f'False : Not Recive ARP {TestIPv4_} by lan2 MAC use 0.0.0.0 check IP used {TestIPv4_}')
        assert lan2_.ARPBlockCheck(srcIP=TestIPv4_,dstIP=lan2_.gatewayIp,ProbeMAC=ProbeMAC_)\
            ,(AthenacWebAPI_.DelIP(ip=TestIPv4_,siteid=SiteID_),f'False : Not Recive ARP {TestIPv4_} by lan2 MAC use {TestIPv4_}')
        AthenacWebAPI_.DelIP(ip=TestIPv4_,siteid=SiteID_)
    ...
    def test_BindingIP(self)->None:
        ...
        lan2_.SendARPReply(IP=TestIPv4_,Count=2,WaitSec=2)
        assert lan2_.ARPBlockCheck(srcIP=TestIPv4_,dstIP=lan2_.gatewayIp,ProbeMAC=ProbeMAC_)\
            ,(AthenacWebAPI_.DelIP(ip=lan2_.Ip,siteid=SiteID_),f'False : Not Recive ARP {TestIPv4_} by lan2 MAC use {TestIPv4_}')
        AthenacWebAPI_.DelIP(ip=lan2_.Ip,siteid=SiteID_)
        assert not lan1_.ARPBlockCheck(srcIP=TestIPv4_,dstIP=lan1_.gatewayIp,ProbeMAC=ProbeMAC_)\
            ,f'False : Recive ARP {TestIPv4_} by lan1 MAC use {TestIPv4_}'

class cancelTestNACbyRadius:
    def test_by_External(self)->None:
        ...
        dynamicset = RadiusSetting(
            SiteId=SiteID_
            ,EnableRadius=True
            ,EnableDynamicVLAN=True
            ,EnableExternalAutoQuarantine=True
            ,EnableInternalAutoQuarantine=True
            ,SiteVerifyModule = SiteVerifyModule.EnableDbVerify.value
        )
        ...
        pass
    def test_CoA_by_External_Quart(self)->None:
        ...
        assert lan1replyvlanid == str(dynamicset.ExternalDefaultVLan),f'False : Recive not VLAN ID {dynamicset.ExternalDefaultVLan} is VLAN ID {lan1replyvlanid} from External Default VLAN'
        lan1_.SendDHCPv4Offer() #Known DHCPv4 test by external MAC
        time.sleep(10)
        assert listens.radiuspackets,'False : not Recive CoA Packet from External Quarantine VLAN by DHCPv4'
        listens.radiuspackets.clear()
        lan1replyvlanid = lan1_.GetRadiusReply(serverIP_,lan1_.Ip)
        if lan1replyvlanid:
            lan1replyvlanid = lan1replyvlanid['VLANId']
        assert lan1replyvlanid == str(dynamicset.ExternalQuarantineVLan)\
        ...
        time.sleep(10)
        assert listens.radiuspackets,'False : not Recive CoA Packet from External Quarantine VLAN by DHCPv6'
        listens.radiuspackets.clear()
        lan1replyvlanid = lan1_.GetRadiusReply(serverIP_,lan1_.Ip)
        if lan1replyvlanid:
            lan1replyvlanid = lan1replyvlanid['VLANId']
    ...
    # def UnknowDHCPTestCase(self)->None:
    ...
    # def OutofVLANTestCase(self)->None:
    ...
    # def IPconflictTestCase(self)->None:
    ...
    # def DHCPpressureTestCase(self)->None:
    ...
    # def DHCPv6pressureTestCase(self)->None:
    ...
    # def RadiusDynamicVLANTestCase(self)->None:
    ...
    # def RadiusCoATestCasebyQuar(self)->None:
